...
- [x] 1. Set up project structure, dependencies, and core interfaces
  - [x] 1.1 Create directory structure and install dependencies
    ...
    - Install runtime dependencies: `cytoscape` (bundled for webview), `cytoscape-dagre`, `dagre`
    - Install dev dependencies: `@types/cytoscape`, `fast-check`
  ...
  - [x] 1.2 Define core TypeScript interfaces and types
    ...
    - Node ID format: `{relativePath}#{symbolName}` (e.g., `src/cart/calc.ts#calculateTotal`); for class methods use `ClassName.methodName` as the symbol name
...
- [x] 2. Implement Code Parser
  - [x] 2.1 Implement `parseProject` in `src/parser/codeParser.ts`
    ...
    - Skip dynamic calls (string-keyed property access, `eval`, framework-injected callbacks) and document these as known limitations
...
- [x] 3. Implement Graph Builder
  - [x] 3.1 Implement `buildGraph` in `src/graph/graphBuilder.ts`
    ...
    - Ensure graph invariants: no duplicate nodes, referential integrity, forward/reverse symmetry (including self-edges for recursive functions)
  ...
  - [ ]* 3.2 Write unit tests for Graph Builder
    ...
    - Test self-edges (recursive functions) are preserved
...
- [x] 6. Implement Impact Engine
  - [x] 6.1 Implement `computeBlastRadius` in `src/analysis/impactEngine.ts`
    ...
    - Must complete and return results within 200 milliseconds
...
- [x] 7. Implement Call Graph serialization
  - [x] 7.1 Implement serialization helpers in `src/graph/serialization.ts`
    - Implement `serializeCallGraph(graph: CallGraph): SerializedCallGraph`: convert `nodes` Map and adjacency Maps/Sets to plain arrays for JSON transmission
    - Implement `serializeTestMap(testMap: TestMap): SerializedTestMap`: convert `mapping` Map/Sets to array of `{ testFile, functionNodeIds }` entries
    - Implement `deserializeCallGraph(serialized: SerializedCallGraph): { nodes: SerializedFunctionNode[], edges: SerializedCallEdge[] }` for use in the webview client
  ...
  - [ ]* 7.2 Write unit tests for serialization round-trip
    ...
    - Test graph with self-edges (recursive functions)
    - _Requirements: 12.3_

- [x] 8. Implement Webview Provider and Panel Lifecycle
  - [x] 8.1 Implement `BlastRadiusPanel` in `src/webview/webviewProvider.ts`
    ...
    - Generate HTML content inline (not from a file) that loads `cytoscape.min.js`, `cytoscape-dagre.js`, and `graph.js` from `media/` using webview URIs
    - Set Content Security Policy in the HTML `<meta>` tag to restrict network access to `vscode-resource:` and `file:` schemes only
    - Implement `show(graph, testMap)`: call serialization helpers, send `graph-data` message via `postMessage`
    ...
    - _Requirements: 6.1, 6.2, 7.1, 7.2, 8.1, 8.3, 11.3, 12.1, 12.2_

  - [x] 8.2 Implement data privacy enforcement
    - At panel initialization, verify the Content Security Policy blocks all non-`vscode-resource:` and non-`file:` network requests
    - Throw an error if any external network URIs are detected in the webview configuration
    - Ensure no source code, file names, or graph data are transmitted outside the extension host process
    - _Requirements: 11.1, 11.2, 11.3_

- [x] 9. Implement Webview Client (Cytoscape.js Rendering)
  - [x] 9.1 Create webview JavaScript and bundle Cytoscape assets
    - Create `media/graph.js` with Cytoscape.js initialization and VSCode `acquireVsCodeApi()` message handling
    - Copy/bundle `cytoscape.min.js` and `cytoscape-dagre.js` (plus `dagre.js`) into `media/` folder
    - _Requirements: 6.1, 6.3_

  - [x] 9.2 Implement graph rendering from `graph-data` messages
    ...
    - Must render a graph containing 200 nodes at 30 frames per second or higher
    - _Requirements: 6.1, 6.3, 6.4, 6.5, 7.2_

  - [x] 9.3 Implement node click handling and highlight application
  ...
  - [x] 9.4 Implement Impact Summary sidebar rendering
    - On receiving `highlight` message, populate the Impact Summary section in the webview HTML
    - Display count of affected function nodes
    - List recommended Jest test file paths
    - List at-risk feature modules
    - _Requirements: 9.1, 9.2, 9.3, 9.4_

- [x] 10. Wire everything together in the command handler
  - [x] 10.1 Update `src/extension.ts` with the full pipeline
...
- Cytoscape.js, cytoscape-dagre, and dagre are bundled in `media/` for the webview; no runtime network dependencies
- The extension uses the standard VSCode `postMessage` API for host↔webview communication
- Webview HTML is generated inline by `BlastRadiusPanel` (not loaded from a file); only `graph.js` and Cytoscape assets live in `media/`
